...
namespace Sharpy {
    /// <summary>
    ///    Inherits from Generator class with the type signature &lt;T, Randomizer&gt;.
    /// </summary>
    /// <typeparam name="T"></typeparam>
    public class SharpyGenerator<T> : Generator<T, Randomizer> {
        /// <summary>
        ///    Uses my own implementation of IRandomizer. Has an optional config argument as secondary argument.
        /// </summary>
        public SharpyGenerator(Func<IRandomizer<StringType>, T> func, Config config = null)
            : base(func, new Randomizer(config ?? new Config())) {}

        /// <summary>
        ///    Uses my own implementation of. Has an optional config argument as secondary argument.
