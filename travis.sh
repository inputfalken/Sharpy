...
if ! wget -O .nuget/nuget.exe https://dist.nuget.org/win-x86-commandline/latest/nuget.exe
then
  echo "${green}Download Successfull${reset}"
fi

echo "${yellow}Restoring solution with NuGet${reset}"
if ! mono .nuget/nuget.exe restore Sharpy.sln -Verbosity quiet 
then
...
if ! mono .nuget/nuget.exe install NUnit.Runners -Version 3.6.1 -OutputDirectory testrunner -Verbosity quiet
then
...
if ! msbuild /v:minimal /p:Configuration=Release Sharpy.sln
then
